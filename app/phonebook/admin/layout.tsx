...
import { 
    ...
    ShieldCheck, ChevronDown, CheckCircle, UserPen, History, PhoneCall, Lock, LayoutDashboard, UserRoundCog
} from 'lucide-react';
...
const navItems = [
    {
        name: 'หน้าหลัก',
        href: '/phonebook/admin/dashboard',
        icon: LayoutDashboard,
    },
    {
        name: 'ค้นหาบุคลากร',
        href: '/phonebook/admin/personal',
        icon: User,
    },
    {
        name: 'ค้นหาสำนักงาน',
        href: '/phonebook/admin/department',
        icon: Building,
    },
    {
        name: 'จัดการบุคลากร',
        href: '/phonebook/admin/manageUser',
        icon: UserPen,
    },
    {
        name: 'ประวัติกิจกรรม',
        href: '/phonebook/admin/dashboard/activity-logs',
        icon: UserRoundCog,
    },
    {
        name: 'จัดการสำนักงาน',
        href: '/phonebook/admin/manageDepartment',
        icon: Building2 ,
    },
        {
        name: 'ประวัติแก้ไขสำนักงาน',
        href: '/phonebook/admin/historyDept',
        icon: History,
    },
    {
        name: 'เปลี่ยนรหัสผ่าน',
        href: '/phonebook/admin/changePassword',
        icon: Lock,
    },
];
...
const Sidebar = ({ isSidebarOpen, toggleSidebar, onLinkClick }) => {
    ...
    <Link href="/phonebook/admin/dashboard" className="flex items-center">
            ...
            <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                ...
                return (
                        <Link
                            key={item.name}
                            href={item.href}
                            onClick={onLinkClick} // เพิ่มบรรทัดนี้
                            className={`
                                flex items-center px-3.5 py-2.5 mb-2 rounded-lg transition-all duration-200
                                ${isActive 
                                    ? 'bg-orange-600 hover:bg-orange-700 text-white shadow-md' 
                                    : 'text-gray-600 hover:bg-orange-50 hover:text-orange-600'}
                            `}
                        >
                            <Icon size={24} className={isSidebarOpen ? 'mr-3' : 'mx-auto'} />
...
// ------------------------------------------------------------------
const Header = ({ toggleSidebar, isSidebarOpen, fullname }) => { // ✅ รับ isSidebarOpen มาเพื่อปรับไอคอน
    ...
    return (
        <header className="sticky top-0 flex-shrink-0 h-16 bg-white shadow-md border-b fixed">
            <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div className="text-lg font-semibold text-gray-800 hidden sm:block">
                        <Link href="/phonebook/auth/personal">
            ...
            <div className="flex items-center space-x-4">
                ...
                <div className="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 border border-red-200">
                    <User size={20} />
                </div>
            </div>
            </div>
...
export default function SearchLayout({ children }: { children: React.ReactNode }) {
    ...
    const [isLoadingPage, setIsLoadingPage] = useState(false); // เพิ่มตัวแปรนี้
    ...
    // ระบบจะปิด Loading อัตโนมัติเมื่อ useEffect ตรวจพบว่า pathname เปลี่ยน (หน้าใหม่โหลดเสร็จ)
    };

        // ใช้ useEffect ตรวจจับการเปลี่ยน Path เพื่อปิด Loading
    useEffect(() => {
        setIsLoadingPage(false);
        
        // ✅ ดึงข้อมูล user จาก localStorage เมื่อหน้าเว็บโหลด
    ...
    return (
        <div className="flex h-screen bg-gray-100 overflow-hidden">
            ...
            <Sidebar 
                ...
                onLinkClick={handleLinkClick} // เพิ่มบรรทัดนี้
            />
            ...
            >
                ...
                <Header 
                    toggleSidebar={toggleSidebar} 
                    isSidebarOpen={isSidebarOpen}
                    fullname={user?.fullname || 'กำลังโหลด...'}
                />

                {/* Page Content (children: page.tsx) */}
                <main className="flex-1 overflow-y-auto bg-gray-100 p-6 scroll-smooth relative">
                    {/* Loading Overlay */}
                    {isLoadingPage && (
                        <div className="absolute inset-0 z-50 flex items-center justify-center bg-gray-100/60 backdrop-blur-[1px] transition-all">
                    ...
                    <div className={`max-w-7xl mx-auto transition-opacity duration-300 ${isLoadingPage ? 'opacity-30' : 'opacity-100'}`}>
                        {children}
                    </div>
                </main>
