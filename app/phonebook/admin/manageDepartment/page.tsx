...
interface AccountEntry {
    ...
    initial: string;           // ชื่อย่อหน่วยงาน
                ...
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input name="initial" value={formData.initial} onChange={handleChange}
                                type="text" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                        <div>
...
const ManageDepartment = () => {
    ...
    const [filterStatus, setFilterStatus] = useState<string>('ALL'); // 'ALL', 'ACTIVE', 'INACTIVE', 'PENDING'
        ...
        setLoading(true);
        try {
            const res = await fetch('/api/admin/manageDepartment');
 ...
 const filteredAccounts = useMemo(() => {
    // เปลี่ยนจาก mockAccountData เป็น accounts
    return accounts.filter(account => {
        const matchesSearch = 
            // ตรวจสอบว่าชื่อ property ตรงกับที่มาจาก API (officeName, adminName)
...
}, [searchTerm, filterStatus, accounts]); // เพิ่ม accounts ใน dependency
            ...
            <div className="bg-white rounded-lg shadow overflow-x-auto">
                <table className="w-full table-fixed divide-y divide-gray-200 bg-white">
                    <thead className="bg-gray-50">
                        <tr>
                            <th onClick={() => handleSort('officeName')} className="w-[30%] px-6 py-3 text-left text-sm font-medium text-gray-900 uppercase tracking-wider cursor-pointer hover:bg-gray-100">
                            ...
                            <th onClick={() => handleSort('adminName')} className="w-[20%] px-6 py-3 text-left text-sm font-medium text-gray-900 uppercase tracking-wider cursor-pointer hover:bg-gray-100">
                                <div className="flex items-center space-x-1">
                            ...
                            <th className="w-[20%] px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">เบอร์โทร/อีเมล</th>
                            <th onClick={() => handleSort('status')} className="w-[15%] px-6 py-3 text-center text-sm font-medium text-gray-900 uppercase tracking-wider cursor-pointer hover:bg-gray-100">
                            ...
                            <th className="w-[12%] px-6 py-3 text-center text-sm font-medium text-gray-500 uppercase tracking-wider">จัดการ</th>
                        </tr>
                    </thead>
                                ...
                                return (
                                    <tr key={item.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <p className="text-sm font-semibold text-gray-900">{item.officeName} ({item.initial})</p>
