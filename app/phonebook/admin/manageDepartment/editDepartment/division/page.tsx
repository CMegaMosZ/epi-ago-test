...
export default function ManageSubDepartments() {
    ...
    // เพิ่ม State เหล่านี้ใน ManageDepartment
    const [isPersonnelModalOpen, setIsPersonnelModalOpen] = useState(false);
    const [selectedDivision, setSelectedDivision] = useState<any>(null);
    const [personnelList, setPersonnelList] = useState<any[]>([]); // ข้อมูลคนในกลุ่มที่เลือก
    const [subDepartments, setSubDepartments] = useState<SubDepartment[]>([
        { id: Date.now(), name: '', initials: '', phone: '' }
    ]);

    const handleManagePersonnel = (division: any) => {
    setSelectedDivision(division);
    // ในอนาคตต้อง Fetch ข้อมูลจริงจาก API โดยใช้ division.id
    // ตัวอย่าง Mock data:
    setPersonnelList([
        { id: 1, name: 'สมชาย สายตรวจ', position: 'เจ้าหน้าที่ IT', phone: '101' },
        { id: 2, name: 'สมหญิง จริงใจ', position: 'ธุรการ', phone: '102' },
    ]);
